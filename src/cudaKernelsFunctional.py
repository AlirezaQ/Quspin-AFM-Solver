...
import sys
import cudah as hfield
import cudaJ as J_curr
...
@cuda.jit(device=True)
def cudaExchange2(m_1, m_2, w_ex, A_1, A_2, border_size, dx,dim):
    i = cuda.blockDim.x*cuda.blockIdx.x + cuda.threadIdx.x
    return -(w_ex*m_2[i,dim] -A_1*cudaLaplacian1D(m_1, border_size, dim)/dx**2 - A_2*cudaLaplacian1D(m_2, border_size, dim))/dx**2
...
@cuda.jit(device =True)
def cudaf(m_1, m_2,timestep,is_a,res, border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J,d,h,D, T, rand,dx):
    ...
    #Load constants 
    i = cuda.blockDim.x*cuda.blockIdx.x + cuda.threadIdx.x
    res[i,0] = cudaExchange2(m_1, m_2, w_ex, A_1, A_2, border_size,dx, 0) + cudaZeeman(i,h,border_size, 0) + cudaAnisotropy1D(m_1,m_2,w_1,w_2,0) + cudaDMIEnergy1D(m_2,is_a,d,0) + D*CudaSecondDMI1D(m_1,0) + T[i,0]*rd.xoroshiro128p_normal_float64(rand, i)
    res[i,1] = cudaExchange2(m_1, m_2, w_ex, A_1, A_2, border_size,dx, 1) + cudaZeeman(i,h, border_size,1) + cudaAnisotropy1D(m_1,m_2,w_1,w_2,1) + cudaDMIEnergy1D(m_2,is_a,d,1) + D*CudaSecondDMI1D(m_1,1) + T[i,1]*rd.xoroshiro128p_normal_float64(rand, i)
    res[i,2] = cudaExchange2(m_1, m_2, w_ex, A_1, A_2, border_size,dx, 2) + cudaZeeman(i,h, border_size,2) + cudaAnisotropy1D(m_1,m_2,w_1,w_2,2) + cudaDMIEnergy1D(m_2,is_a,d,2) + D*CudaSecondDMI1D(m_1,2) + T[i,2]*rd.xoroshiro128p_normal_float64(rand, i)
...
@cuda.jit(device =True)
def cudaf2D(m_1, m_2,geom,timestep,is_a,res, border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J_args,d,h_args,D,Temp,rand):
    """Calculates mdot, ie f(m_1,m_2,i) which is used in the RK4 routine. 
    ...
    i = cuda.blockDim.x*cuda.blockIdx.x + cuda.threadIdx.x
    j =  cuda.blockDim.y*cuda.blockIdx.y + cuda.threadIdx.y
    res[i,j,0] = cudaExchange2D(m_1,m_2,geom,w_ex,A_1,A_2,0) + hfield.h(m_1,h_args,timestep,0) + cudaAnisotropy2D(m_1, m_2, w_1, w_2, 0) + Temp[i,j,0]*rd.xoroshiro128p_normal_float64(rand, i+j*m_1.shape[0]) + cudaDMIEnergy2D(m_1, is_a, d,0) + D*cudaSecondDMI2D(m_1,0)
    res[i,j,1] = cudaExchange2D(m_1,m_2,geom,w_ex,A_1,A_2,1) + hfield.h(m_1,h_args,timestep,1) + cudaAnisotropy2D(m_1, m_2, w_1, w_2, 1) + Temp[i,j,1]*rd.xoroshiro128p_normal_float64(rand, i+j*m_1.shape[0]) + cudaDMIEnergy2D(m_1, is_a, d,1) + D*cudaSecondDMI2D(m_1,1)
    res[i,j,2] = cudaExchange2D(m_1,m_2,geom,w_ex,A_1,A_2,2) + hfield.h(m_1,h_args,timestep,2) + cudaAnisotropy2D(m_1, m_2, w_1, w_2, 2) + Temp[i,j,2]*rd.xoroshiro128p_normal_float64(rand, i+j*m_1.shape[0]) + cudaDMIEnergy2D(m_1, is_a, d,2) + D*cudaSecondDMI2D(m_1,2)
    ...
    if i >= 0 and i <m_1.shape[0] and j>=0 and j<m_1.shape[1]:
        ...
        beta_z = beta/K_z
        #H_1 + C*J[:,i,:] + alpha[:,:]*(crossProduct(m_1, H_1 +beta*J[:,i,:]))
        temp_x = res[i,j,0] + C*J_curr.J(m_1,J_args, timestep,geom, 0)+ alpha[i,j,0]*((m_1[i,j,1]*(res[i,j,2] + beta_x*J_curr.J(m_1,J_args,timestep,geom,2)) - m_1[i,j,2]*(res[i,j,1] + beta_x*J_curr.J(m_1,J_args,timestep,geom,1))))
        temp_y = res[i,j,1] + C*J_curr.J(m_1,J_args, timestep,geom, 1)+ alpha[i,j,1]*((m_1[i,j,2]*(res[i,j,0] + beta_x*J_curr.J(m_1,J_args,timestep,geom,0)) - m_1[i,j,0]*(res[i,j,2] + beta_x*J_curr.J(m_1,J_args,timestep,geom,2))))
        temp_z = res[i,j,2] + C*J_curr.J(m_1,J_args, timestep,geom, 2)+ alpha[i,j,2]*((m_1[i,j,0]*(res[i,j,1] + beta_x*J_curr.J(m_1,J_args,timestep,geom,1)) - m_1[i,j,1]*(res[i,j,0] + beta_x*J_curr.J(m_1,J_args,timestep,geom,0))))
...
def cudaTimeStep(m_1, m_2,k1,k2,k3,k4,dt,i, is_a, border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J,d,h,D,T,rand,dx,res, resdot):
    ...
    N_x = m_1.shape[0]
    if j >= 0:
        k1 = cudaf(m_1[:], m_2[:],i,is_a,k1, border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J,d,h,D,T, rand,dx)
        res[j,0] = m_1[j,0] + k1[j,0]*0.5*dt
        res[j,1] = m_1[j,1] + k1[j,1]*0.5*dt
        res[j,2] = m_1[j,2] + k1[j,2]*0.5*dt
    
  
        k2 = cudaf(res, m_2[:], i,is_a,k2, border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J,d,h,D,T,rand,dx)
        res[j,0] = m_1[j,0] + k2[j,0]*0.5*dt
        res[j,1] = m_1[j,1] + k2[j,1]*0.5*dt
        res[j,2] = m_1[j,2] + k2[j,2]*0.5*dt
    
    
        k3 = cudaf(res, m_2[:], i,is_a,k3, border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J,d,h,D,T,rand,dx)
        ...
        k4 = cudaf(res, m_2[:], i,is_a,k4, border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J,d,h,D,T,rand,dx)
...
@cuda.jit(fastmath = True)    
def cudaTimeStep2D(m_1, m_2,geom,k1,k2,k3,k4,dt,stride,t_i, is_a, border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J,d,h,D,Temp,rand,res_a, resdot_a,res_b,resdot_b):
    """A timestep of the RK45 routine. Uses f(m_1,m_2,i) to calculate a timestep
    ...
    for k in range(stride):               
        if (j >= 0) and (j < N_y) and (i>=0) and (i < N_x):
            ...
            #First find the results for the A sublattice
            k1 = cudaf2D(m_1[:,:], m_2[:,:],geom,t_i +k,True,k1, border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J,d,h,D,Temp, rand)
            res_a[i,j,0] = m_1[i,j,0] + k1[i,j,0]*0.5*dt
            res_a[i,j,1] = m_1[i,j,1] + k1[i,j,1]*0.5*dt
            res_a[i,j,2] = m_1[i,j,2] + k1[i,j,2]*0.5*dt
        
    
            k2 = cudaf2D(res_a, m_2[:],geom, t_i +k,True,k2, border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J,d,h,D,Temp,rand)
            res_a[i,j,0] = m_1[i,j,0] + k2[i,j,0]*0.5*dt
            res_a[i,j,1] = m_1[i,j,1] + k2[i,j,1]*0.5*dt
            res_a[i,j,2] = m_1[i,j,2] + k2[i,j,2]*0.5*dt
        
        
            k3 = cudaf2D(res_a, m_2[:,:],geom, t_i +k,True,k3, border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J,d,h,D,Temp,rand)
        
            res_a[i,j,0] = m_1[i,j,0] + k3[i,j,0]*dt
            res_a[i,j,1] = m_1[i,j,1] + k3[i,j,1]*dt
            res_a[i,j,2] = m_1[i,j,2] + k3[i,j,2]*dt
        
        
            k4 = cudaf2D(res_a, m_2[:,:],geom, t_i +k,True,k4, border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J,d,h,D,Temp,rand)
        
        
        #m_1[:] += (dt / 6) * (k1 + 2* k2 + 2 * k3 + k4)
        
            res_a[i,j,0] = m_1[i,j,0] + (dt / 6) * (k1[i,j,0] + 2* k2[i,j,0] + 2 * k3[i,j,0] + k4[i,j,0])
            res_a[i,j,1] = m_1[i,j,1] + (dt / 6) * (k1[i,j,1] + 2* k2[i,j,1] + 2 * k3[i,j,1] + k4[i,j,1])
            res_a[i,j,2] = m_1[i,j,2] + (dt / 6) * (k1[i,j,2] + 2* k2[i,j,2] + 2 * k3[i,j,2] + k4[i,j,2])

            #normalize
            abs_val = sqrt(res_a[i,j,0]**2 + res_a[i,j,1]**2 + res_a[i,j,2]**2)
            
            if geom[i,j,0] != 0:
                res_a[i,j,0] = res_a[i,j,0]/abs_val
                res_a[i,j,1] = res_a[i,j,1]/abs_val
                res_a[i,j,2] = res_a[i,j,2]/abs_val
            else:
                res_a[i,j,0] = 0
                res_a[i,j,1] = 0
                res_a[i,j,2] = 0

            resdot_a[i,j,0] = k1[i,j,0]
            resdot_a[i,j,1] = k1[i,j,1]
            resdot_a[i,j,2] = k1[i,j,2]
            cuda.syncthreads()

            #Then find the results in the B sublattice
            k1 = cudaf2D(m_2[:,:], m_1[:,:],geom,t_i+k,False,k1, border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J,d,h,D,Temp, rand)
            res_b[i,j,0] = m_2[i,j,0] + k1[i,j,0]*0.5*dt
            res_b[i,j,1] = m_2[i,j,1] + k1[i,j,1]*0.5*dt
            res_b[i,j,2] = m_2[i,j,2] + k1[i,j,2]*0.5*dt
        
    
            k2 = cudaf2D(res_b, m_1[:],geom, t_i +k,False,k2, border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J,d,h,D,Temp,rand)
            res_b[i,j,0] = m_2[i,j,0] + k2[i,j,0]*0.5*dt
            res_b[i,j,1] = m_2[i,j,1] + k2[i,j,1]*0.5*dt
            res_b[i,j,2] = m_2[i,j,2] + k2[i,j,2]*0.5*dt
        
        
            k3 = cudaf2D(res_b, m_1[:,:],geom, t_i+k,False,k3, border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J,d,h,D,Temp,rand)
        
            res_b[i,j,0] = m_2[i,j,0] + k3[i,j,0]*dt
            res_b[i,j,1] = m_2[i,j,1] + k3[i,j,1]*dt
            res_b[i,j,2] = m_2[i,j,2] + k3[i,j,2]*dt
        
        
            k4 = cudaf2D(res_b, m_1[:,:],geom, t_i+k,False,k4, border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J,d,h,D,Temp,rand)
        
        
        #m_2[:] += (dt / 6) * (k1 + 2* k2 + 2 * k3 + k4)
        
            res_b[i,j,0] = m_2[i,j,0] + (dt / 6) * (k1[i,j,0] + 2* k2[i,j,0] + 2 * k3[i,j,0] + k4[i,j,0])
            res_b[i,j,1] = m_2[i,j,1] + (dt / 6) * (k1[i,j,1] + 2* k2[i,j,1] + 2 * k3[i,j,1] + k4[i,j,1])
            res_b[i,j,2] = m_2[i,j,2] + (dt / 6) * (k1[i,j,2] + 2* k2[i,j,2] + 2 * k3[i,j,2] + k4[i,j,2])

            #normalize
            abs_val = sqrt(res_b[i,j,0]**2 + res_b[i,j,1]**2 + res_b[i,j,2]**2)
            
            if geom[i,j,0] != 0:
                res_b[i,j,0] = res_b[i,j,0]/abs_val
                res_b[i,j,1] = res_b[i,j,1]/abs_val
                res_b[i,j,2] = res_b[i,j,2]/abs_val
            else:
                res_b[i,j,0] = 0
                res_b[i,j,1] = 0
                res_b[i,j,2] = 0

            resdot_b[i,j,0] = k1[i,j,0]
            resdot_b[i,j,1] = k1[i,j,1]
            resdot_b[i,j,2] = k1[i,j,2]
            cuda.syncthreads()


            #Need to load results back into the mag arrays
            m_1[i,j,0] = res_a[i,j,0]
            m_1[i,j,1] = res_a[i,j,1]
            m_1[i,j,2] = res_a[i,j,2]
             
 
            m_2[i,j,0] = res_b[i,j,0]
            m_2[i,j,1] = res_b[i,j,1]
            m_2[i,j,2] = res_b[i,j,2]

            #One of the threads iterate t_i
...
def cudaTimeEvolution(m_a, m_b, N_steps, dt, cut_borders_at = 0, border_size = 0,stride = 0, constants = ()):
    ...
    w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J,d,h,D, Temp,dx = constants
    ...
    with cuda.defer_cleanup():
        for i in tqdm(range(N_steps)):
            ...
            cudaTimeStep[blockspergrid,threadsperblock](temp_m_a, temp_m_b,d_k1,d_k2,d_k3,d_k4, dt,stride, i*stride, True, border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,d_alpha,d_J,d,d_h,D,d_T,d_rand,dx, res_a, resdot_a)
            temp_m_a, temp_m_a_dot= res_a,resdot_a
            
            cudaTimeStep[blockspergrid,threadsperblock](temp_m_b, inter_m_a,d_k1,d_k2,d_k3,d_k4, dt,stride, i*stride, False,border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,d_alpha,d_J,d,d_h,D,d_T,d_rand,dx, res_b, resdot_b)
...
def cudaTimeEvolution2D(m_a,m_b, N_steps, dt, geom, cut_borders_at=0, border_size = 0, stride = 0, constants = ()):
    ...
    N_x = m_a.shape[0]
    N_y = m_a.shape[1]
    blockspergrid_x = (N_x +(block_size_x -1))// block_size_x
    blockspergrid_y = (N_y +(block_size_y -1))// block_size_y
    blockspergrid = blockspergrid_x,blockspergrid_y
    threadblock = block_size_x, block_size_y
    j = 0
    #Initialising loop arrays
    temp_m_a = np.zeros((m_a.shape[0], 3))
    temp_m_b = np.zeros_like(temp_m_a)
    ...
    res_a = np.ascontiguousarray(res_a)
    resdot_a = np.ascontiguousarray(resdot_a)
    res_b = np.ascontiguousarray(np.zeros_like(res_a))
    resdot_b = np.ascontiguousarray(np.zeros_like(res_a))
    ...
    d_k3 = cuda.to_device(k3)
    d_k4 = cuda.to_device(k4)
    w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J_args,d,h_args,D, Temp = constants
    d_geom = np.ascontiguousarray(geom)
    d_geom = cuda.to_device(d_geom)
    
    #Unpacking J arguments here
    J_arr = np.asarray(J_args)
    d_J = cuda.to_device(J_arr)

    #Same for h arguments
    h_arr = np.asarray(h_args)
    ...
    for i in tqdm(range(int(N_steps//stride))):
        if int(i//stride) == cut_borders_at:
            border_size = 0    
            
        cudaTimeStep2D[blockspergrid,threadblock](temp_m_a, temp_m_b,d_geom,d_k1,d_k2,d_k3,d_k4, dt,stride, i*stride, True, 
        border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,d_alpha,d_J,d,d_h,D,d_T,d_rand, res_a, resdot_a,res_b,resdot_b)
        #temp_m_a, temp_m_a_dot= res_a,resdot_a
            
        """cudaTimeStep2D[blockspergrid,threadblock](temp_m_b, inter_m_a,d_geom,d_k1,d_k2,d_k3,d_k4, dt,stride, i, False,border_size, w_ex,A_1,A_2,w_1,w_2,beta,C,d_alpha,d_J,d,d_h,D,d_T,d_rand,res_b, resdot_b)"""
        #temp_m_b, temp_m_b_dot= res_b,resdot_b
