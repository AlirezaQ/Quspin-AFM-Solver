# -*- coding: utf-8 -*-
"""
Created on Fri Dec 25 22:35:57 2020
@author: event and Viroshau
"""
import numpy as np
import numba as nb
import time
...
import cudaKernelsFunctional as ckf
import random as rd
import functionalMethod as funcmet
...
@nb.jit(cache = True, nopython = True)
def laplacian2D(mesh,border_size =0):
...
def exchangeEnergy2D(m_a_element, m_b_element, w_ex, A_1, A_2, border_size=0):
    ...
    ex_array = np.zeros_like(m_a_element,dtype = dtype)
    ex_array = -(w_ex*m_b_element[:,:]) -A_1*laplacian2D(m_a_element) - A_2*laplacian2D(m_b_element)
...
@nb.jit(cache = True, nopython = True)
def f(m_1,m_2,i, a, init = False, border_size=0, constants = ()):
    ...
    H_1 = exchangeEnergy(m_1, m_2,w_ex,A_1, A_2, border_size=border_size)  + zeemanEnergy(h,i, border_size=border_size) + anisotropyEnergy1D(m_1, m_2,w_1,w_2,border_size=border_size) + DMIEnergy(m_2, a, d) + D*secondDMI1D(m_1, border_size=border_size) + H_T
    ...
    f_1 = -(K)*crossProduct(m_1, H_1 + C*J[:,i,:] + alpha[:,:]*(crossProduct(m_1, H_1 +beta*J[:,i,:])))
    return f_1, f_1

@nb.jit(cache = True, nopython = True)
def f2D(m_1,m_2,i, a, init = False, border_size=0, constants = ()):
    """[Calculates mdot, i.e f(m_1,m_2,i) which is used in the RK4 routine]
    ...
    H_1 = np.zeros_like(m_1, dtype = dtype)
    H_1 = exchangeEnergy2D(m_1, m_2,w_ex,A_1, A_2, border_size=border_size) + anisotropyEnergy2D(m_1,m_2, w_1, w_2, border_size = border_size) + zeemanEnergy2D(h,i) + DMIEnergy2D(m_2,a,d) + H_T + D*secondDMI2D(m_1, border_size = border_size) 
    ...
    f_1 = np.zeros_like(m_1, dtype = dtype)
    f_1[border_size:f_1.shape[0]-border_size,border_size:f_1.shape[1]-border_size] = -(K[border_size:f_1.shape[0]-border_size,border_size:f_1.shape[1]-border_size])*crossProduct2D(m_1[border_size:f_1.shape[0]-border_size,border_size:f_1.shape[1]-border_size], H_1[border_size:f_1.shape[0]-border_size,border_size:f_1.shape[1]-border_size] + C*J[border_size:f_1.shape[0]-border_size,border_size:f_1.shape[1]-border_size,i,:] + alpha[border_size:f_1.shape[0]-border_size,border_size:f_1.shape[1]-border_size]*(crossProduct2D(m_1[border_size:f_1.shape[0]-border_size,border_size:f_1.shape[1]-border_size], H_1[border_size:f_1.shape[0]-border_size,border_size:f_1.shape[1]-border_size] +beta[border_size:f_1.shape[0]-border_size,border_size:f_1.shape[1]-border_size]*J[border_size:f_1.shape[0]-border_size,border_size:f_1.shape[1]-border_size,i,:])))
    return f_1, f_1

@nb.jit(cache = True, nopython = True)
def timeStep(m_1, m_2, dt,i, a, border_size=0, constants = ()):
    ...
    """
    k1,mdot = f(m_1[:], m_2[:],i,a, border_size=border_size, constants = constants)
    
    k2,_ = f(np.add(m_1[:, :], k1*0.5*dt), m_2[:], i,a, border_size=border_size, constants = constants)
    
    k3,_ = f(np.add(m_1[:, :], 0.5*k2*dt), m_2[:], i,a,border_size=border_size, constants = constants)
    
    k4,_ = f(np.add(m_1[:, :], dt*k3), m_2[:], i,a, border_size=border_size, constants = constants)
...
@nb.jit(cache = True, nopython = True)
def timeStep2D(m_1, m_2, dt,i, a, border_size=0, constants = ()):
    ...
    """
    k1,mdot = f2D(m_1, m_2,i,a, border_size=border_size, constants = constants)
    
    k2,_ = f2D(np.add(m_1, k1*0.5*dt), m_2, i,a, border_size=border_size, constants = constants)
    
    k3,_ = f2D(np.add(m_1, 0.5*k2*dt), m_2, i,a,border_size=border_size, constants = constants)
    
    k4,_ = f2D(np.add(m_1, dt*k3), m_2, i,a, border_size=border_size, constants = constants)
...
def timeEvolution(m_a, m_b, N_steps, dt, cut_borders_at = 0, border_size = 0, stride = 0, constants = ()):
    ...
    for i in tqdm(range(N_steps)):
        if i == cut_borders_at:
            border_size = 0
        inter_m_a = temp_m_a
        temp_m_a, temp_m_a_dot= timeStep(temp_m_a, temp_m_b, dt, i, True, border_size=border_size, constants = constants)
        temp_m_b, temp_m_b_dot= timeStep(temp_m_b, inter_m_a, dt, i, False,border_size=border_size, constants = constants)
        if i%stride == 0:
            T[j] += (i)*dt
            m_a[:,j,:] = temp_m_a
...
def timeEvolution2D(m_a, m_b, N_steps, dt, cut_borders_at = 0, border_size = 0, stride = 0, constants = ()):
    ...
    for i in tqdm(range(N_steps)):
        if i == cut_borders_at:
            border_size = 0
        inter_m_a = temp_m_a
        temp_m_a, temp_m_a_dot = timeStep2D(temp_m_a, temp_m_b, dt, i, True, border_size=border_size, constants = constants)
        temp_m_b, temp_m_b_dot = timeStep2D(temp_m_b, inter_m_a, dt, i, False,border_size=border_size, constants = constants)
        if (i)%stride == 0:
            T[j] += (i)*dt
            m_a[:,:,j,:] = temp_m_a
            mdot_a[:,:,j,:] = temp_m_a_dot
...
def solver1D(N_steps, dt, init_state,  cut_borders_at = 0, border_size = 0, stride = 0, constants = ()):
    ...
    m_a, m_b, T,mdot_a,mdot_b = timeEvolution(m_a_start, m_b_start, N_steps, dt, cut_borders_at = cut_borders_at, border_size = border_size, constants = constants, stride = stride)
    return m_a, m_b, T, mdot_a, mdot_b

def solver2D(N_steps, dt, init_state,  cut_borders_at = 0, border_size = 0, stride = 0, constants = ()):
    """A wrapper function that is needed to call to both initialize the system as well as solve it
    ...
    """
    m_a_start, m_b_start = init_state[0], init_state[1]
    m_a, m_b, T,mdot_a,mdot_b = timeEvolution2D(m_a_start, m_b_start, N_steps, dt, cut_borders_at = cut_borders_at, border_size = border_size,stride = stride, constants = constants)
    return m_a, m_b, T, mdot_a, mdot_b

def runSimulation(Nx,N_steps,dt,init_state,geom,Ny = 0, Nz = 0, cut_borders_at = -1, border_size = 0, stride = 0, target = "cpu",functional = False, constants = ()):
    """This is the main function that performs a complete simulation. It solves the LLG equation using RK4aaaaa routine.
    After the spin-configuration are calculated for all times, the user can specify what to plot and animate. 

    Args:
        Nx ([int]): Number of lattice sites in the x-direction
        N_steps ([int]): Number of timesteps
        dt ([float]): Size of timestep used in rk4 method
        init_state ([np.ndarray(Nx,Ny,N_steps,3)]): 2 arrays of size (Nx,Ny,N_steps,3) where the first time-element represents the initial state of m_a and m_b respectively
        Ny (int, optional): Number of lattice sites in the y-direction. If set to 0, it solves a 1D problem instead of a 2D lattice problem. 
        Nz (int, optional):  Number of lattice sites in the z-direction. If set to 0, it solves a 2D problem instead of a 3D lattice problem. 
        cut_borders_at (int, optional): [description]. Defaults to 0.
        border_size (int, optional): Size of the border of the system where the spins remain fixed. Defaults to 0.
        stride (int, optional): The number of timesteps that are skipped before the state of the system is saved. Defaults to 0.
        target (str, optional): It set to cpu, it runs the program using a cpu. If 'cuda' is called, the program is run on cuda-gpu. Defaults to "cpu".
        constants (tuple, optional): [description]. Defaults to ().

    Returns:
        The sizes of the returns depends on wether the system is a 1D or 2D system.
        m_a ([np.ndarray(Nx,Ny,N_steps//stride,3)]): The spins at the A-sublattice at all times
        m_b ([np.ndarray(Nx,Ny,N_steps//stride,3)]): The spins at the B-sublattice at all times
        T ([np.ndarray(N_steps//stride)]): The times of the simulated system
        mdot_a ([np.ndarray(Nx,Ny,N_steps//stride,3)]): The timederivative of the spins at the A-sublattice at all times
    ...
    if target.lower() == "cuda" or target.lower() == "gpu":
        cuda.select_device(1)
        if Ny == 0 and Nz == 0 and functional == False:
            m_a, m_b, T, mdot_a, mdot_b = ck.cudaSolver1D(N_steps,dt,init_state, cut_borders_at=cut_borders_at, border_size = border_size, stride = stride, constants = constants)
        elif Ny != 0 and functional == False:
            m_a,m_b, T, mdot_a, mdot_b = ck.cudaSolver2D(N_steps, dt,init_state, geom, cut_borders_at = cut_borders_at, border_size = border_size, stride= stride, constants = constants)
        elif Ny != 0 and functional == True:
            w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J_func, J_args,d,h_func, h_args,D, Temp = constants 
            constants = (w_ex,A_1,A_2,w_1,w_2,beta,C,alpha,J_args,d,h_args,D, Temp)
            m_a, m_b, T, mdot_a, mdot_b = ckf.cudaSolver2D(N_steps, dt,init_state, geom, cut_borders_at = cut_borders_at, border_size = border_size, stride= stride, constants = constants)
        else:
            raise TypeError("Functional method not implemented for cuda version of program")
    elif target.lower() == "cpu":
        #Run simulation
        if Ny == 0 and Nz == 0 and not functional:
            m_a, m_b, T,mdot_a,mdot_b = solver1D(N_steps,dt,init_state, cut_borders_at=cut_borders_at, border_size = border_size, stride = stride, constants = constants)
        elif Ny != 0 and not functional:
            m_a, m_b, T, mdot_a, mdot_b = solver2D(N_steps,dt,init_state,cut_borders_at = cut_borders_at, border_size= border_size, stride = stride, constants = constants)
        elif Ny == 0 and Nz == 0 and functional:
            m_a, m_b, T,mdot_a,mdot_b = funcmet.solver1D(N_steps,dt,init_state,geom,cut_borders_at=cut_borders_at, border_size = border_size, stride = stride, constants = constants)
        elif Ny != 0 and functional:
            m_a, m_b, T, mdot_a, mdot_b = funcmet.solver2D(N_steps,dt,init_state,geom,cut_borders_at = cut_borders_at, border_size= border_size, stride = stride, constants = constants)
        else:
            print("3D her")
    else:
        raise TypeError("No target recognized, available targets are: \"cuda\" and \"cpu\"")        
    #Get end time measurement
    end_time = time.time()
    print("Spent time: \n", end_time -start_time)

    return m_a, m_b, mdot_a, mdot_b, T
